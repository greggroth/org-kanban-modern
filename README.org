...
- *Tags on cards, elfeed-style filtering.*  Tags appear as clickable chips that
  ...
  carry every included tag); right-click (=mouse-3=) to *exclude* it (cards
  ...
  for excludes — click one to remove it.
...
| =mouse-3= (card tag)  | Toggle that tag in the *exclude* filter   |
