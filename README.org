...
- *Sorting.*  Within each column, cards are sorted by Org priority (highest
  first, unprioritized last) by default; set ~org-kanban-modern-sort~ to
  ~document~ for source order, or to a predicate for a custom order.
...
- ~org-kanban-modern-sort~ — order of cards within each column.  ~priority~
  (default) sorts highest Org priority first, then unprioritized; equal
  priorities keep document order.  ~document~ keeps source order.  May also be
  a predicate of two cards (passed to ~sort~).
